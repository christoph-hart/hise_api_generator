...
forumReferences:
  - id: 1
    title: "Preset loading stops the arpeggiator via allNotesOff"
    summary: "Loading a user preset sends an allNotesOff message which kills all voices and stops the arpeggiator; to preserve a pattern across a preset change, store and retrigger the notes manually after load."
    topic: 6109
  - id: 2
    title: "Artificial arp notes are invisible to sibling MIDI processors"
    summary: "Notes generated by the arpeggiator cannot be intercepted by sibling MIDI script processors in the same chain due to an internal recursion guard; place the arp in a parent container so child processors can see its output."
    topic: 7288
  - id: 3
    title: "Arp timer does not stop when DAW transport stops"
    summary: "The arpeggiator timer continues running when the DAW is stopped and only stops when held notes are released; use a TransportHandler to detect stop events and release notes manually."
    topic: 4633
customEquivalent:
  approach: hisescript
...
Notes generated by the Arpeggiator are marked as artificial events and are not visible to sibling MIDI processors in the same chain. To process arp-generated notes in a downstream script, place the Arpeggiator in a parent container's MIDI chain so that child sound generators' MIDI processors can see the output. [2]($FORUM_REF.7288$)
...
The arpeggiator timer does not respond to DAW transport - it continues running as long as notes are held, even when the host is stopped. Use a TransportHandler to detect stop events and release held notes if transport-aware behaviour is needed. [3]($FORUM_REF.4633$)

Loading a user preset sends an all-notes-off message that stops the arpeggiator. To maintain a running pattern across preset changes, store the held note state before loading and retrigger the notes after the preset has loaded. [1]($FORUM_REF.6109$)
