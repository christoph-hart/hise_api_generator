...
> [!Warning:$WARNING_TO_BE_REPLACED$] Always pair with `playNoteFromUI`, never with `playNote` or `addNoteOn`. Notes injected through the UI pipeline are real events with no event ID tracking. Mixing UI and artificial note-off methods causes stuck notes.
