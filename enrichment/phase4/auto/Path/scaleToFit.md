...
> **Warning:** The transformation is permanent. If you need to query the path later (e.g. with `getYAt`) in pixel space, call `scaleToFit` first, then query. The path cannot be "unscaled" back to its original coordinates - rebuild it if you need the original space.
