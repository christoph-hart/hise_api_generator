...
> **Warning:** When using `addArc` with normalised coordinates `[0, 0, 1, 1]` for later scaling via `drawPath`, the path's bounding box only covers the arc segment itself. Call `setBounds([0, 0, 1, 1])` before the arc to anchor the bounds to the full unit square, preventing skewed or misaligned rendering.
