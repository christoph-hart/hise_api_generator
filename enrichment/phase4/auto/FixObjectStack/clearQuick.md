...
> **Warning:** `toBase64()` serialises the full memory block including slots cleared by `clearQuick()`. Call `clear()` before serialising if stale data must not leak into the output.