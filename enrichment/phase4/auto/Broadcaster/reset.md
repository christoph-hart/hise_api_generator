...
> **Warning:** For broadcasters created with the standard `{ id: "...", args: [...] }` format, all default values are `undefined`, so `reset()` has no visible effect - the undefined-argument check suppresses the dispatch.
