...
> **Warning:** State function results are cached and only refreshed after a menu selection or an explicit `refreshContextMenuState()` call. If external state changes between menu openings, call `refreshContextMenuState()` to update tick marks and enabled states.

> **Warning:** This attachment does not override existing right-click behaviour. Disable `enableMidiLearn` on attached components to prevent the MIDI learn popup from conflicting with the context menu.
