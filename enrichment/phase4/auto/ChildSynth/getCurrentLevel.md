...
> **Warning:** These are display-rate values, not sample-accurate measurements. Apply decay smoothing (e.g. `level = Math.max(newPeak, level * 0.85)`) for stable meter display - raw values fluctuate rapidly between timer ticks.
