...
> **Warning:** When setting `LoVel` and `HiVel` on the same sample, auto-clipping can interfere. Setting `LoVel` above the current `HiVel` clamps it down. Set both values twice in succession to ensure both survive the interdependent range clamping.
