...
> **Warning:** If the data slot index exceeds the number of slots the module provides, the method silently returns `undefined` with no error. Check the return value with `isDefined()` before use.