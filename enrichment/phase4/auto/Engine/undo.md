...
> **Warning:** Non-script undo operations complete after `undo()` returns. Do not read state immediately after calling `undo()` and expect it to reflect the reverted value.