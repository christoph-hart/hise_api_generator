...
> [!Warning:$WARNING_TO_BE_REPLACED$] Only the single most recent call is tracked. If your workflow makes multiple sequential requests, only the last one can be retried with this method. For multi-step flows, store the step state and re-invoke the appropriate function directly.
