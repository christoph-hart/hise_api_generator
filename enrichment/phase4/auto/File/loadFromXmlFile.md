...
> [!Warning:$WARNING_TO_BE_REPLACED$] Returns `undefined` silently on any failure (missing file, invalid XML, empty content). Unlike `loadAsObject`, no script error is reported. Always guard with `isDefined()`.
