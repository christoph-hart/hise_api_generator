...
> [!Warning:$WARNING_TO_BE_REPLACED$] Reports a script error if the file exists but contains invalid JSON. This differs from `loadEncryptedObject` and `loadFromXmlFile`, which silently return `undefined` on parse failure.
