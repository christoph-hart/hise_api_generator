...
> **Warning:** Passing a non-File object as `otherFile` silently returns `false` instead of reporting an error, which can mask bugs where a string path is passed by mistake.
