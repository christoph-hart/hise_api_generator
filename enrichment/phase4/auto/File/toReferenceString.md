...
> **Warning:** The method constructs the reference string regardless of whether the file actually resides inside the specified subdirectory. If the file is outside the folder, the returned string will not resolve correctly when used with `FileSystem.fromReferenceString`.
