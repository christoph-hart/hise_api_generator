...
> **Warning:** Returns `undefined` silently if the file does not exist or is not a recognised audio format. Unlike `loadAsObject`, no script error is reported on failure. Always check with `isDefined()` before accessing properties.
