...
> **Warning:** The `xy` parameter is a `[x, y]` point (2 elements), not an `[x, y, w, h]` area like most other Graphics methods. The `y` value is the baseline position, so the first line of text appears slightly above the y coordinate.
