...
> [!Warning:$WARNING_TO_BE_REPLACED$] In the HISE IDE, call `Engine.loadAudioFilesIntoPool()` before using `{PROJECT_FOLDER}` references. Exported plugins embed audio files automatically and do not need this step.
