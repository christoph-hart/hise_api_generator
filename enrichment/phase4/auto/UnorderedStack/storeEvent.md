...
> **Warning:** When draining the stack, always read and remove from index 0 in a `while (!stack.isEmpty())` loop. A forward-iterating for loop skips elements because `removeElement()` swaps the last element into the vacated slot.
