...
> **Warning:** Gesture collisions are resolved by the first internal match, and matching order is not stable. Avoid overlapping mappings with the default map unless you redefine the conflicting actions explicitly.

> **Warning:** Use `mods.noKeyModifier` (singular). Some older examples show `noKeyModifiers`, which does not match the current constant name.
