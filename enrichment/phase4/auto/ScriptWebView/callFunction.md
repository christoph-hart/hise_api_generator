...
> [!Warning:$WARNING_TO_BE_REPLACED$] With `enablePersistence` set to `true`, all `callFunction` calls are recorded and replayed when a new webview instance is created. Timer-driven calls replay only the most recent value for each function name, but other calls accumulate.
