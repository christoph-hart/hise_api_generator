...
> [!Warning:$WARNING_TO_BE_REPLACED$] Timer callbacks run on the message thread, not the audio thread. They are safe for UI operations but not suitable for sample-accurate timing. For beat-synced callbacks, use `TransportHandler` instead.
