...
> [!Warning:$WARNING_TO_BE_REPLACED$] Once an event is ignored, re-enabling it with `ignoreEvent(false)` only works if downstream processors have not already skipped it. In deferred callback mode, ignored events are skipped before the deferred callback runs, so re-enabling has no effect.
