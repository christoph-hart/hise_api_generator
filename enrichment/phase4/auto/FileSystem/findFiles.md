...
> **Warning:** Passing a `SpecialLocations` constant (e.g. `FileSystem.Desktop`) instead of a `File` object silently returns an empty array. Always call `FileSystem.getFolder()` first.

> **Warning:** On large content libraries, recursive scanning can take noticeable time. Cache the results and only rescan when the user explicitly requests it.
