...
**Disabled Reason:** The `macroControl` property is deactivated for ScriptImage.
...
**Disabled:** redundant
**Disabled Reason:** Base implementation is a trivial pass-through; normalized value mapping is only meaningful on ScriptSlider.
...
**Disabled:** no-op
**Disabled Reason:** Not registered as a direct API method on component instances. Use `Content.setPropertiesFromJSON()` instead.
...
**Description:**
Returns the component's ID as a string (the variable name used when creating the component, e.g. `"MyImage"`).
...
**Description:**
Attaches a scripted look and feel object to this component and all its children. Pass `undefined` to clear it. The object must be a `ScriptedLookAndFeel` instance. If the object is not a valid LAF, the local look and feel is cleared. Note that ScriptImage has no LAF drawing functions -- CSS rendering via `StyleSheetLookAndFeel` can intercept the image drawing via `drawImageOnComponent()`.
...
**Signature:** `undefined setImageFile(String absoluteFileName, Integer forceUseRealFile)`
**Return Type:** `undefined`
**Call Scope:** unsafe
**Minimal Example:** `{obj}.setImageFile("{PROJECT_FOLDER}myImage.png", 0);`

**Description:**
Sets the image file to display. The file name is resolved via `PoolReference` with the project's `Images/` folder. Loading goes through `ExpansionHandler::loadImageReference()`, which supports both project images and expansion images. Pass an empty string to clear the image. After loading, any active blend mode is recomputed via `updateBlendMode()`.

**Parameters:**

| Name | Type | Forced | Description | Constraints |
|------|------|--------|-------------|-------------|
| absoluteFileName | String | no | The image file reference (relative to the Images/ folder or expansion path) | Must be a valid image pool reference, or "" to clear |
| forceUseRealFile | Integer | no | Intended to force reloading from disk instead of cache | Currently ignored -- has no effect |
...
// Title: Loading an image from the project's Images folder
const var img = Content.addImage("MyImage", 0, 0);
img.setImageFile("{PROJECT_FOLDER}background.png", 0);
```
```json:testMetadata:set-image-file
{
  "testable": false,
  "skipReason": "Requires an image file in the project's Images folder"
}
```
...
**Signature:** `undefined setAlpha(Double newAlphaValue)`
**Return Type:** `undefined`
**Call Scope:** unsafe
**Minimal Example:** `{obj}.setAlpha(0.5);`

**Description:**
Sets the transparency of the displayed image. The alpha value is clamped to the 0.0-1.0 range during rendering (0.0 = fully transparent, 1.0 = fully opaque). Sets the `alpha` property with change notification, triggering a UI repaint.

**Parameters:**

| Name | Type | Forced | Description | Constraints |
|------|------|--------|-------------|-------------|
| newAlphaValue | Double | no | The opacity value | 0.0 (fully transparent) to 1.0 (fully opaque) |

**Property Links:**
- Equivalent: set("alpha", newAlphaValue)
- Related: get("alpha")
