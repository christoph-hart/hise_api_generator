...
- The interpreted wrapper uses `extra_config_with_display::prepare()` to call `ScriptnodeExceptionHandler::validateMidiProcessingContext(parentNode)`. Because `mod_base::isProcessingHiseEvent()` returns true, `extra_mod` needs either a polyphonic root network or a `container.midichain` ancestor, with no `container.no_midi` ancestor. For frame processing, the legal order is `midichain -> frame*_block -> extra_mod`; `midichain` itself throws `IllegalFrameCall` inside frame contexts.
- `extra_config_with_display::checkIndex()` also validates `root->getParameterProperties().isUsed(index)`. `ParameterProperties::fromValueTree()` only marks slots as used when root parameters have `ExternalModulation` set to a mode such as `Combined`, `Gain`, `Offset`, `Pan`, or `Pitch`. Without that root parameter assignment, the node reports `No parameter assigned to modulation slot #N`.
