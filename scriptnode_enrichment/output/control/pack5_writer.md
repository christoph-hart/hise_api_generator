...
llmRef: |
  ...
  Writes five individual parameter values into a connected slider pack. Variant of pack2_writer with 5 Value parameters instead of 2. See control.pack2_writer for full details.

  Signal flow:
    Control node - no audio processing
    Value1 -> sliderPack[0] ... Value5 -> sliderPack[4]

  CPU: negligible, monophonic

  Parameters:
    Value1-Value5: 0.0 - 1.0 (default 0.0) -> slider pack indices 0-4

  When to use:
    Populating a 5-element slider pack from individual parameter connections or modulation sources.
...
The pack5_writer writes five individual parameter values into a connected slider pack. It is a variant of [control.pack2_writer]($SN.control.pack2_writer$) with 5 Value parameters instead of 2. Each parameter maps to a slider index: Value1 to index 0 through Value5 to index 4.

When connected, the slider pack is automatically resized to 5 entries. The node has no modulation output - it writes directly to the slider pack data. See [control.pack2_writer]($SN.control.pack2_writer$) for full behavioural details.
...
```
// control.pack5_writer - writes 5 parameters to slider pack
