...
commonMistakes:
  - title: "Not suitable for modulated delay effects"
    ...
    explanation: "Thiran interpolator uses internal allpass filter whose state needs time to settle after coefficient changes. Rapid delay time modulation causes audible artefacts."
  - title: "No built-in delay time smoothing"
    ...
    explanation: "Delay time is applied immediately without ramping. Abrupt changes produce audible clicks."
llmRef: |
  jdsp.jdelay_thiran

  Delay line using Thiran allpass interpolation. Flat amplitude response with low CPU cost, but NOT suitable for fast delay time modulation -- allpass filter produces artefacts when its coefficients change rapidly. Polyphonic.
  ...
  When to use:
    Fixed or slowly changing delays where flat frequency response matters but fast modulation not needed (e.g., comb filters, static delays in reverb networks). For modulated effects, use jdelay_cubic instead.
...
Delay line using Thiran allpass interpolation to calculate values between samples. Like cubic variant, this produces flat amplitude response with no high-frequency roll-off. CPU cost is comparable to linear interpolation, making it efficient choice for applications that need tonal accuracy.

However, Thiran interpolator uses internal allpass filter that has state which needs time to settle after coefficient changes. Rapidly modulating delay time causes filter to produce transient artefacts, making this variant unsuitable for chorus, flanger, or other modulated delay effects. It is best suited for static or slowly changing delay times such as comb filters or fixed delays in reverb feedback networks.

Each voice maintains own delay buffer. When used polyphonically, maximum delay time is reduced from 1000 ms to 30 ms.

> [!Warning:Not for fast delay modulation] Thiran allpass filter produces artefacts when delay time changes rapidly. For chorus or flanger effects, use [jdelay_cubic]($SN.jdsp.jdelay_cubic$) instead.
...
glossary:
  ...
  functions:
    thiranDelay:
      desc: "Reads from delay buffer using Thiran allpass interpolation"
---
