...
Real-time pitch shift. Change pitch, keep duration. Two stages: resample input at FreqRatio rate, then time stretch back to original speed. Output plays at original tempo, shifted pitch.

FreqRatio = multiplicative pitch factor. 1.0 = pass. 2.0 = +1 oct. 0.5 = -1 oct. Full range two octaves each way (0.25 – 4.0). Knob skewed, 1.0 at centre.

Good quality for moderate shifts (~1 octave). Extreme ratios near 0.25 or 4.0 = phasiness, spectral smear. Some latency from overlap-add windowing.
...
At FreqRatio=1.0 resampling 1:1. Time stretcher gets equal in/out sample count. Effective pass-through, minimal overhead.
