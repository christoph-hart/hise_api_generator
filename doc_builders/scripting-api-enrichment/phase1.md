...
**ASCII-only rule:** All Phase 1 output files (`Readme.md`, `methods_todo.md`, `methods.md`) must use ASCII characters only. Use `--` instead of em-dashes, straight quotes instead of curly quotes, etc. The Write tool on Windows may produce CP1252 instead of UTF-8 for non-ASCII characters, which breaks the merge script's UTF-8 parser.
