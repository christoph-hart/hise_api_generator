...
**Purpose:** Convert approved Phase 3 construction artifacts into public `.hsc` example scripts. This phase is a deterministic formatting/conversion step and must not reinterpret topology decisions.
...
2. Do not reinterpret graph topology, parameter values, comments, or cosmetics.
3. Convert only the approved Phase 3 optimized public shell command sequence.
4. Exclude `save` from public `.hsc`.
5. Exclude `screenshot` from public `.hsc`.
6. Preserve friction-point comments and place them near the relevant commands.
7. Use `add <type> as "Name"` for all creation commands.
8. Use `set X.field value` or `set X.Param.field value` for writes.
9. Use `create_parameter Container.Param [min, max] default D stepSize S` for macro parameters.
10. Use `connect Source.Param to Target.Param matched` whenever Phase 3 says matched was used.
11. Use only `0xAARRGGBB` colour literals.
12. Ensure `scriptnode_enrichment/hsc/phase4/{factory}/` exists before writing `.hsc` files.
13. Enter DSP mode with `/dsp`, then select the host module with `cd <ModuleId>`. Do not emit `/dsp <ModuleId>`.
14. Translate parameter metadata as `.stepSize`, `.middlePosition`, and `.skewFactor`. Do not emit `.step`, `.mid`, or `.skew`.
15. Preserve DSP appearance writes such as `NodeColour`, `Comment`, and `Folded`; these are valid node attributes used for screenshot-focused examples.
